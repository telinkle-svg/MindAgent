...
MindAgent 是一个教学型 Agent 应用，用于演示从模型调用、Agent 循环、工具调度到知识库检索的完整链路。项目包含 Spring Boot 后端和 React 前端，当前代码可以作为后续重构、功能扩展和工程化测试的基线。
...
后端通过环境变量读取敏感配置。不要把真实密码、API Key 或邮箱授权码写入 Git 仓库。
