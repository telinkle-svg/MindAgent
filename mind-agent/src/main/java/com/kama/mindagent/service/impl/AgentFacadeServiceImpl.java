...
public class AgentFacadeServiceImpl implements AgentFacadeService {
    ...
    public CreateAgentResponse createAgent(CreateAgentRequest request) {
        try {
            ...
            if (result <= 0) {
                throw BizException.internalServerError("创建 agent 失败");
            }
            ...
            throw BizException.internalServerError("创建 agent 时发生序列化错误: " + e.getMessage());
        }
    }
    ...
    public void deleteAgent(String agentId) {
        ...
        if (result <= 0) {
            throw BizException.internalServerError("删除 agent 失败");
        }
    }
    ...
    public void updateAgent(String agentId, UpdateAgentRequest request) {
        try {
            ...
            if (result <= 0) {
                throw BizException.internalServerError("更新 agent 失败");
            }
        } catch (JsonProcessingException e) {
            throw BizException.internalServerError("更新 agent 时发生序列化错误: " + e.getMessage());
        }
    }
}
