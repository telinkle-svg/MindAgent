...
public class DocumentStorageServiceImpl implements DocumentStorageService {
    ...
    public String saveFile(String kbId, String documentId, MultipartFile file) throws IOException {
        ...
        try {
            ...
            // 保存文件
            targetPath = documentDir.resolve(uniqueFilename);
            ...
            cleanupFailedSave(targetPath, documentDir);
            throw exception;
        }
    }

    private void cleanupFailedSave(Path targetPath, Path documentDir) {
        try {
            if (targetPath != null) {
            ...
            log.error("保存文件失败后清理半成品失败: targetPath={}", targetPath, cleanupException);
        }
    }
