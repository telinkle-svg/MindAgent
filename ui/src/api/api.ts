...
export async function uploadDocument(
  ...
  let apiResponse: ApiResponse<CreateDocumentResponse> | null = null;
  try {
    apiResponse = (await response.json()) as ApiResponse<CreateDocumentResponse>;
  } catch {
    // 非 JSON 响应由下方统一回退到 HTTP 状态信息。
  }

  if (!response.ok || !apiResponse || apiResponse.code !== 200) {
    throw new Error(
